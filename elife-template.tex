...
We can take a slight shortcut if we use vector notation.

We define $\mathbf{q}$ as a vector of charges $N$, $q_i$ for each state $i$.
%
Then, if we take $\mathbf{pi}$ to be a matrix of $N$ states by $M$ pH values,
%
We can simply take the matrix product to achieve the same.
\begin{align}
    \langle q_\text{total} \rangle (\pH) = \mathbf{q} \mathbf{\pi}
\end{align}
